...
import ArmazemRequests from '../persistence/armazemRequests';
import LoggerInstance from './logger';

export default ({ mongoConnection, schemas, requests, controllers, repos, services }: {
  mongoConnection;
  schemas: { name: string; schema: any }[],
  requests: { name: string; path: string }[],
  ...
  try {
    ...
    let requestsClass = new ArmazemRequests();//require(m.path).ArmazemRequests;
    Container.set(requests[0].name, requestsClass);
